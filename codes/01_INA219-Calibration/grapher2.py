...
# cell area
# not used during I and V calibration of INA219
area = 1.0
...
#transfer curve difference
mask = df['type'].str.startswith('JV')
df.loc[mask, 'difference'] = df.loc[mask].groupby('integer4725')['voltage'].diff()

#df['difference'] = df.groupby('integer4725')['voltage'].diff()
